...
class AutoApplyService {
  async handleMatchedJob(userId: string | Types.ObjectId, jobId: string | Types.ObjectId, score: number) {
    ...
    const requireReview = user.requireReview !== false;

    if (requireReview || normalizedScore < threshold) {
